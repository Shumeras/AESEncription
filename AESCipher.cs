using System;
using System.Collections.Generic;
using System.Text;
using AESEncryption;

// Currently only the 128bit key size complies with the standard
public enum KeySize
{
    bit128,
    bit192,
    bit256
}

class AESCipher
{
    #region LookupTables

    static readonly byte[] sBox =  {
        
    };

    static readonly byte[] rSBox = {
        , /*0xa8*/0xc8, 0xeb, 0xbb, 0x3c, 0x83, 0x53, 0x99, 0x61,
        0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d
    };

    static readonly byte[] mul2 = {
        
    };

    static readonly byte[] mul3 = {
        
    };

    static readonly byte[] mul9 =
    {
        
    };

    static readonly byte[] mul11 = {
        
    };

    static readonly byte[] mul13 = {
        
    };

    static readonly byte[] mul14 = {
        
    };

    static readonly byte[] rcon = {
        
    };

    #endregion

    public KeySize KeySize { get; private set; }
    public byte[] Key { get; private set; }
    public byte[] ExpandedKey { get; private set; }

    public AESCipher(byte[] key, KeySize size = KeySize.bit128)
    {
        SetKey(key, size);
    }

    public void SetKey(byte[] key, KeySize size = KeySize.bit128)
    {
        KeySize = size;
        if(key.Length < 16)
        {
            var temp = new byte[16];
            for(int i = 0; i<16; i++)
            {
                if(i < key.Length)
                    temp[i] = key[i];
                else
                    temp[i] = 0x00;
            }
            key = temp;
        }
        Key = key;
        ExpandedKey = KeyExpand(key, size == KeySize.bit128 ? 176 : 
                                    size == KeySize.bit192 ? 208 : 240);
    }

    private byte[] KeyExpand(byte[] key, int targetSize)
    {

        byte[] expKey = new byte[targetSize];

        for(int i = 0; i< 16; i++)
            expKey[i] = key[i];

        // We generate 4 bytes at a time and every 16 generated bytes we increment iteration (for rcon)
        for(int bytesGenerated = 16, iteration = 1; bytesGenerated < targetSize; bytesGenerated+=4)
        {
            byte[] temp = new byte[4];

            for(int i = 0; i < 4; i++)
                temp[i] = expKey[(bytesGenerated - 4) + i];

            //Every new iteration we run expansion core
            if(bytesGenerated % 16 == 0)
                temp = KeyExpandCore(temp, iteration++);
            
            //After we have temp values we (DO NOT FORGET TO!) xor with values of same block in previous generation 
            expKey[bytesGenerated+0] = (byte)(temp[0] ^ expKey[bytesGenerated+0-16]);
            expKey[bytesGenerated+1] = (byte)(temp[1] ^ expKey[bytesGenerated+1-16]);
            expKey[bytesGenerated+2] = (byte)(temp[2] ^ expKey[bytesGenerated+2-16]);
            expKey[bytesGenerated+3] = (byte)(temp[3] ^ expKey[bytesGenerated+3-16]);
        }

        if(Params.verbose) System.Console.WriteLine("Expanded key: ");
        if(Params.verbose) System.Console.WriteLine(Program.BytesToFormatedString(expKey));
        return expKey;
    }

    private byte[] KeyExpandCore(byte[] key, int i)
    {
        //We shift keys to the left (1byte) and then substitute them using sbox
        key = new byte[] {
            sBox[key[1]],
            sBox[key[2]],
            sBox[key[3]],
            sBox[key[0]]
        };

        key[0] ^= rcon[i]; 

        return key;
    }


    #region Encryption

    public byte[] Encrypt(byte[] data)
    {
        List<byte> result = new List<byte>();

        int numberOfRounds = 
            KeySize == KeySize.bit128 ? 10 :
            KeySize == KeySize.bit192 ? 12 :
            14;

        for(int bytesEncoded = 0; bytesEncoded <= data.Length; bytesEncoded += 16)
        {
            //Taking a block of data (16 bytes) from plaintext
            #region CreatBlock
            
            byte[] currentFragment = new byte[16];

            if(bytesEncoded+16 <= data.Length)
            {
                for(int i = 0; i < 16; i++)
                    currentFragment[i] = data[bytesEncoded+i];
            }
            else
            {
                if(!Params.usePadding)
                {
                    if(bytesEncoded == data.Length)
                        break;
                    else
                    {
                        System.Console.WriteLine("Warning: padding is turned off and text contains a non-full block. Cutting off");
                        break;
                    }
                }
                byte remainder = (byte)(16-(data.Length % 16));
                
                if(remainder == 0)
                    remainder = 16;

                for(int i = 0; i < 16; i++ )
                {
                    if(i < 16-remainder)
                        currentFragment[i] = data[bytesEncoded+i];
                    else
                        currentFragment[i] = remainder;
                }

            }
            
            if (Params.verbose) Console.WriteLine("New block: \t" + Program.BytesToFormatedString(currentFragment));

            #endregion

            //CBC mode XOR'ing with initvec or last encripted block;
            #region CBC

            if(Params.useCBC)
            {
                byte[] scramble;

                if(bytesEncoded == 0)
                {
                    scramble = Encoding.UTF8.GetBytes(Params.initVector);
                }
                else
                {
                    scramble = new byte[16];
                    for(int i = 0; i < 16; i++)
                    {
                        scramble[i] = result[bytesEncoded-16+i];
                    }
                }

                for(int i = 0; i < 16; i++)
                {
                    currentFragment[i] ^= scramble[i];
                }

                if(Params.verbose)
                {
                    System.Console.WriteLine("CBC:");
                    System.Console.WriteLine("Scrambling bytes: \t" + Program.BytesToFormatedString(scramble));
                    System.Console.WriteLine("Scrambled fragment: \t" + Program.BytesToFormatedString(currentFragment));
                }
            }
            #endregion CBC

            //AES encript current block
            #region EncryptBlock

            byte[] subKey = new byte[16];
            
            if (Params.verbose) Console.WriteLine("Pre round:");
            
            currentFragment = AddRoundKey(currentFragment, ExpandedKey);

            for(int i = 1; i < numberOfRounds; i++)
            {
                if (Params.verbose) System.Console.WriteLine("Round {0}:", i);
    
                for(int j = 0; j < 16; j++)
                    subKey[j] = ExpandedKey[(16*i)+j];

                if (Params.verbose) System.Console.WriteLine("SubKey: \t" + Program.BytesToFormatedString(subKey));

                currentFragment = SubBytes(currentFragment);
                currentFragment = ShiftRows(currentFragment);  
                currentFragment = MixColumns(currentFragment);
                currentFragment = AddRoundKey(currentFragment, subKey);
                
                if (Params.verbose) System.Console.WriteLine();
    
            }

            for(int i = 0; i < 16; i++)
                subKey[i] = ExpandedKey[(ExpandedKey.Length - 16) + i];

            if (Params.verbose) System.Console.WriteLine("Round {0}:", numberOfRounds);
            if (Params.verbose) System.Console.WriteLine("SubKey: \t" + Program.BytesToFormatedString(subKey));

            currentFragment = SubBytes(currentFragment);
            currentFragment = ShiftRows(currentFragment);
            currentFragment = AddRoundKey(currentFragment, subKey);
            
            if (Params.verbose) System.Console.WriteLine("----------------------------------");


            #endregion
            
            result.AddRange(currentFragment);
        }

        return result.ToArray();
    }

    private byte[] AddRoundKey(byte[] bytes, byte[] roundKey)
    {
        for(int i = 0; i < 16; i++)
            bytes[i] ^= roundKey[i];
        
        if (Params.verbose) System.Console.WriteLine("After add round key: \t" + Program.BytesToFormatedString(bytes));

        return bytes;
    }

    private byte[] SubBytes(byte[] bytes)
    {
        for(int i = 0; i< 16; i++)
            bytes[i] = sBox[bytes[i]];

        if (Params.verbose) System.Console.WriteLine("After sub bytes: \t" + Program.BytesToFormatedString(bytes));

        return bytes; 
    }

    private byte[] ShiftRows(byte[] bytes)
    {
        bytes = new byte[]
        {
            bytes[0], bytes[5], bytes[10], bytes[15],
            bytes[4], bytes[9], bytes[14], bytes[3],
            bytes[8], bytes[13],bytes[2], bytes[7],
            bytes[12],bytes[1], bytes[6], bytes[11]
        };

        if (Params.verbose) System.Console.WriteLine("After shift rows: \t" + Program.BytesToFormatedString(bytes));
        return bytes;
    }
    
    private byte[] MixColumns(byte[] bytes)
    {
        // We use the mul2 & mul3 columns to perform the following matrix (GF)multiplication 
        // 1 - leaves the value; 2 & 3 - performs lshift and xor's (or in GF terms pulinomial multiplication),
        //                               but in this case they are replaced with a table lookup
        // Addition is replaced with XOR
        //  
        //  | 2 3 1 1 |     | byte[0] |
        //  | 1 2 3 1 |  X  | byte[1] |
        //  | 1 1 2 3 |     | byte[2] |
        //  | 3 1 1 2 |     | byte[3] |
        // 
        // We do this with 4 byte matrices (4 times for 16 bytes). Could replace with a for loop and smaller blocks.
        //
        bytes = new byte[]{
            (byte)(mul2[bytes[0]] ^ mul3[bytes[1]] ^ bytes[2] ^ bytes[3]),
            (byte)(bytes[0] ^ mul2[bytes[1]] ^ mul3[bytes[2]] ^ bytes[3]),
            (byte)(bytes[0] ^ bytes[1] ^ mul2[bytes[2]] ^ mul3[bytes[3]]),
            (byte)(mul3[bytes[0]] ^ bytes[1] ^ bytes[2] ^ mul2[bytes[3]]),

            (byte)(mul2[bytes[4]] ^ mul3[bytes[5]] ^ bytes[6] ^ bytes[7]),
            (byte)(bytes[4] ^ mul2[bytes[5]] ^ mul3[bytes[6]] ^ bytes[7]),
            (byte)(bytes[4] ^ bytes[5] ^ mul2[bytes[6]] ^ mul3[bytes[7]]),
            (byte)(mul3[bytes[4]] ^ bytes[5] ^ bytes[6] ^ mul2[bytes[7]]),

            (byte)(mul2[bytes[8]] ^ mul3[bytes[9]] ^ bytes[10] ^ bytes[11]),
            (byte)(bytes[8] ^ mul2[bytes[9]] ^ mul3[bytes[10]] ^ bytes[11]),
            (byte)(bytes[8] ^ bytes[9] ^ mul2[bytes[10]] ^ mul3[bytes[11]]),
            (byte)(mul3[bytes[8]] ^ bytes[9] ^ bytes[10] ^ mul2[bytes[11]]),

            (byte)(mul2[bytes[12]] ^ mul3[bytes[13]] ^ bytes[14] ^ bytes[15]),
            (byte)(bytes[12] ^ mul2[bytes[13]] ^ mul3[bytes[14]] ^ bytes[15]),
            (byte)(bytes[12] ^ bytes[13] ^ mul2[bytes[14]] ^ mul3[bytes[15]]),
            (byte)(mul3[bytes[12]] ^ bytes[13] ^ bytes[14] ^ mul2[bytes[15]])
        };

        if (Params.verbose) System.Console.WriteLine("After mix colums: \t" + Program.BytesToFormatedString(bytes));

        return bytes;
    }

    #endregion


    #region Decryption

    public byte[] Decrypt(byte[] data)
    {
        List<byte> result = new List<byte>();

        int numberOfRounds = 
            KeySize == KeySize.bit128 ? 10 :
            KeySize == KeySize.bit192 ? 12 :
            14;

        for(int bytesDecoded = 0; bytesDecoded < data.Length; bytesDecoded += 16)
        {
            //Taking a block of data (16 bytes) from ciphertext
            #region CreatBlock
            
            byte[] currentFragment = new byte[16];

            if(bytesDecoded+16 <= data.Length)
            {
                for(int i = 0; i < 16; i++)
                    currentFragment[i] = data[bytesDecoded+i];
            }
            else
            {
                Console.WriteLine("Warning: Message is of incorrect length!");
                break;
            }
            #endregion

            #region DecriptBlock

            byte[] subKey = new byte[16];

            for(int i = 0; i < 16; i++)
                subKey[i] = ExpandedKey[(ExpandedKey.Length - 16) + i];

            if(Params.verbose)
            {
                Console.WriteLine("Round: 10:");
                Console.WriteLine("SubKey[ \t \t" + AESEncryption.Program.BytesToFormatedString(subKey)+"];");
                Console.WriteLine("Current fragment: \t" + AESEncryption.Program.BytesToFormatedString(currentFragment));
            }

            currentFragment = AddRoundKey(currentFragment, subKey);
            currentFragment = ReverseShiftRows(currentFragment);
            currentFragment = ReverseSubBytes(currentFragment);
            
            for(int i = numberOfRounds-1; i > 0; i--)
            {
                for(int j = 0; j < 16; j++)
                    subKey[j] = ExpandedKey[(16*i)+j];

                if(Params.verbose) Console.WriteLine("Round: " + i);
                if(Params.verbose) Console.WriteLine("SubKey[ \t \t" + AESEncryption.Program.BytesToFormatedString(subKey)+"];");
                if(Params.verbose) Console.WriteLine("Current fragment: \t" + AESEncryption.Program.BytesToFormatedString(currentFragment));

                currentFragment = AddRoundKey(currentFragment, subKey);
                currentFragment = ReverseMixColumns(currentFragment);
                currentFragment = ReverseShiftRows(currentFragment);
                currentFragment = ReverseSubBytes(currentFragment);

            }

            if(Params.verbose) Console.WriteLine();

            currentFragment = AddRoundKey(currentFragment, ExpandedKey);
            if(Params.verbose) Console.WriteLine("-----------------------------------");
            
            #endregion

            //CBC mode XOR'ing with initvec or last encripted block;
            #region CBC

            if(Params.useCBC)
            {
                byte[] scramble;

                if(bytesDecoded == 0)
                {
                    scramble = Encoding.UTF8.GetBytes(Params.initVector);
                }
                else
                {
                    scramble = new byte[16];
                    for(int i = 0; i < 16; i++)
                    {
                        scramble[i] = data[bytesDecoded-16+i];
                    }
                }

                for(int i = 0; i < 16; i++)
                {
                    currentFragment[i] ^= scramble[i];
                }

                if(Params.verbose)
                {
                    System.Console.WriteLine("CBC:");
                    System.Console.WriteLine("Scrambling bytes: \t" + Program.BytesToFormatedString(scramble));
                    System.Console.WriteLine("Scrambled fragment: \t" + Program.BytesToFormatedString(currentFragment));
                }
            }

            #endregion CBC

            result.AddRange(currentFragment);
        }

        return result.GetRange(0, result.Count - result[result.Count-1]).ToArray();
    }

    private byte[] ReverseSubBytes(byte[] bytes)
    {
        for(int i = 0; i< 16; i++)
            bytes[i] = rSBox[bytes[i]];
        
        if(Params.verbose) Console.WriteLine("After r sub bytes: \t" + AESEncryption.Program.BytesToFormatedString(bytes));

        return bytes; 
    }

    private byte[] ReverseMixColumns(byte[] bytes)
    {
        // Same as mixcolumns, but we are solving for the reverse matrix of the GF polynomial
        // We use the following matrix mul for reverse
        //
        // | 14 11 13 09 |     | bytes[0] |
        // | 09 14 11 13 |  X  | bytes[1] |
        // | 13 09 14 11 |     | bytes[2] |
        // | 11 13 09 14 |     | bytes[3] |

        bytes = new byte[] {
            (byte)(mul14[bytes[0]] ^ mul11[bytes[1]] ^ mul13[bytes[2]] ^ mul9[bytes[3]]),
            (byte)(mul9[bytes[0]]  ^ mul14[bytes[1]] ^ mul11[bytes[2]] ^ mul13[bytes[3]]),
            (byte)(mul13[bytes[0]] ^ mul9[bytes[1]]  ^ mul14[bytes[2]] ^ mul11[bytes[3]]),
            (byte)(mul11[bytes[0]] ^ mul13[bytes[1]] ^ mul9[bytes[2]]  ^ mul14[bytes[3]]),
        
            (byte)(mul14[bytes[4]] ^ mul11[bytes[5]] ^ mul13[bytes[6]] ^ mul9[bytes[7]]),
            (byte)(mul9[bytes[4]]  ^ mul14[bytes[5]] ^ mul11[bytes[6]] ^ mul13[bytes[7]]),
            (byte)(mul13[bytes[4]] ^ mul9[bytes[5]]  ^ mul14[bytes[6]] ^ mul11[bytes[7]]),
            (byte)(mul11[bytes[4]] ^ mul13[bytes[5]] ^ mul9[bytes[6]]  ^ mul14[bytes[7]]),

            (byte)(mul14[bytes[8]] ^ mul11[bytes[9]] ^ mul13[bytes[10]] ^ mul9[bytes[11]]),
            (byte)(mul9[bytes[8]]  ^ mul14[bytes[9]] ^ mul11[bytes[10]] ^ mul13[bytes[11]]),
            (byte)(mul13[bytes[8]] ^ mul9[bytes[9]]  ^ mul14[bytes[10]] ^ mul11[bytes[11]]),
            (byte)(mul11[bytes[8]] ^ mul13[bytes[9]] ^ mul9[bytes[10]]  ^ mul14[bytes[11]]),

            (byte)(mul14[bytes[12]] ^ mul11[bytes[13]] ^ mul13[bytes[14]] ^ mul9[bytes[15]]),
            (byte)(mul9[bytes[12]]  ^ mul14[bytes[13]] ^ mul11[bytes[14]] ^ mul13[bytes[15]]),
            (byte)(mul13[bytes[12]] ^ mul9[bytes[13]]  ^ mul14[bytes[14]] ^ mul11[bytes[15]]),
            (byte)(mul11[bytes[12]] ^ mul13[bytes[13]] ^ mul9[bytes[14]]  ^ mul14[bytes[15]])
        };

        if(Params.verbose) Console.WriteLine("After r mix columns: \t" + AESEncryption.Program.BytesToFormatedString(bytes));

        return bytes;
    }

    private byte[] ReverseShiftRows(byte[] bytes)
    {
        bytes = new byte[] {
            bytes[0], bytes[13],bytes[10],bytes[7],
            bytes[4], bytes[1], bytes[14],bytes[11],
            bytes[8], bytes[5], bytes[2], bytes[15],
            bytes[12],bytes[9], bytes[6], bytes[3]
        };

        if(Params.verbose) Console.WriteLine("After r shift rows: \t" + AESEncryption.Program.BytesToFormatedString(bytes));

        return bytes;

    }


    #endregion

}